...
public class RealBridge implements BridgeInterface, ParameterResolver {
    ...
    public boolean testOpenMarketSystem(String username) {
        ...
        // adminResponse.setReturnValue(true);

        // TokenService tokenServiceMock = mock(TokenService.class);
        // when(tokenServiceMock.extractUsername(token)).thenReturn(username);
        // when(tokenServiceMock.validateToken(token)).thenReturn(true);
        // when(tokenServiceMock.isUserAndLoggedIn(token)).thenReturn(true);

        // UserService userServiceMock = mock(UserService.class);
        // when(userServiceMock.isSystemAdmin(username)).thenReturn(adminResponse);

        // ExternalServiceHandler externalServiceHandlerMock = mock(ExternalServiceHandler.class);
        // when(externalServiceHandlerMock.connectToServices()).thenReturn(true);

        // SystemService _systemServicUnderTest = new SystemService(userServiceMock, _externalServiceHandlerMock,
        // _tokenServiceMock, _userFacadeMock, _shoppingCartFacadeMock);
        // // Act
        // Response res = _systemServicUnderTest.openSystem(token);

        // // Assert
        // System.out.println("TestOpenMarketSystem Error message: " + res.getErrorMessage());
        // return res.getErrorMessage() == null && "System Opened Successfully".equals(res.getReturnValue());
